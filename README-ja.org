...
* scad-ts-mode とは
  ...
  imenu との連携をサポートしています．

  c-mode をベースとしている既存の [[https://github.com/openscad/emacs-scad-mode][scad-mode]] に比べて良い indent を提供するでしょう．
...
** パッケージのインストール
   + TBA
   + MELPA で公開予定です．現状は，clone して ~load-path~ を通して ~(require 'scad-ts-mode)~ を実行して下さい．
...
** パッケージのセットアップ
   ...
   Emacs のバッファから対応する OpenSCAD の GUI インスタンスを実行する例です．
   #+begin_src emacs-lisp
   ...
   file:scripts/openscad-setfocus は，自前のシェルスクリプトで，openscad コマンドによる
   GUI インスタンスがなければ (実行されていなければ) 開き，既にあれば，
   openscad を開かずにフォーカスチェンジをします．
...
** scad-dbus
   Linux 環境で D-Bus が使えるのであれば，[[https://github.com/Lenbok/scad-dbus][Lenbok/scad-dbus]] が便利です．
   #+begin_src emacs-lisp
